...
class LSTM_Optimizee_Model(nn.Module):    
    ...
    def forward(self, input_gradients, prev_state,inputs_data,label):
        ...
        input_gradients=input_gradients+input_temp
        ...
        if prev_state is None: 
            prev_state = (torch.zeros(self.batchsize_para,self.input_size,self.output_size),
                            torch.zeros(self.batchsize_para,self.input_size,self.output_size),
                            torch.zeros(self.batchsize_para,self.input_size,self.output_size),
                            torch.zeros(self.batchsize_para,self.input_size,self.output_size)
                            )        
