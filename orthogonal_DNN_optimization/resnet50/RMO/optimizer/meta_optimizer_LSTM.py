...
class meta_optimizer(nn.Module):
    def __init__(self, opt):
        ...
        self.G_linear_64 = nn.Linear(self.hidden_size*64, self.output_size).cuda()
        self.G_linear_128 = nn.Linear(self.hidden_size*128, self.output_size).cuda()
        self.G_linear_256 = nn.Linear(self.hidden_size * 256, self.output_size)   .cuda()
        self.G_linear_512 = nn.Linear(self.hidden_size * 512, self.output_size)   .cuda()
    ...
    def step(self, params, e_lr, s_lr):
        ...
        for j, s_i in enumerate(params['s_params']):
            ...
            # GtG = G.T.mm(G)
            G_input = self.LogAndSign_Preprocess_Gradient(G)
            # G_input = G_input.reshape(1, -1, 1)

            # L_h0_i = params['L_h0'][j].cuda()
            # L_c0_i = params['L_c0'][j].cuda()
            # R_h0_i = params['R_h0'][j]#.cuda()
            # R_c0_i = params['R_c0'][j]#.cuda()
            G_h0_i = params['R_h0'][j].cuda()
            G_c0_i = params['R_c0'][j].cuda()
            ...
            g_output , (hn, cn)= self.lstm(G_input, (G_h0_i, G_c0_i))
            # l_ = self.L_linear(l_output.squeeze())*self.output_scale #因为LSTM的输出是当前步的Hidden，需要变换到output的相同形状上
            # l_ = l_ - l_.mean() + 1.0
            # L = l_.squeeze().diag()
            # L = torch.diag_embed(L)
            # r_ = self.R_linear(r_output.squ(eeze())*self.output_scale
            # r_ = r_ - r_.mean() + 1.0
            # R = r_.squeeze().diag()
            g_=g_output.squeeze()
            g_ = g_.view(-1)
            if G.shape[1] ==64:
                g_ = self.G_linear_64(g_) * self.output_scale
            elif G.shape[1] ==128:
                g_ = self.G_linear_128(g_) * self.output_scale
            elif G.shape[1] ==256:
                g_ = self.G_linear_256(g_) * self.output_scale
            elif G.shape[1] ==512:
                g_ = self.G_linear_512(g_) * self.output_scale
            # g_ = g_ - g_.mean() + 1.0
            # G_ = g_.squeeze().diag()
            # R = r_.squeeze().diag()
            ...
            new_R_h0.append(hn.detach())
            new_R_c0.append(cn.detach())
            ...
            if new_s_i.requires_grad ==False:
