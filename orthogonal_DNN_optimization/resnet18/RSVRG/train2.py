...
from optimizer.RSVRG import RSVRG
from RB import RB
import numpy as np
import time
...
def data_preprocess():
    ...
    train_dataset = datasets.CIFAR10(root='/home/peilin/cifar10/data/', train=True, transform=transforms.Compose([
        transforms.RandomCrop(32, padding=4),
        transforms.RandomHorizontalFlip(p=0.5),

        transforms.ToTensor(),
        normalize,
        Cutout()]),download=False)
    test_dataset = datasets.CIFAR10(root='/home/peilin/cifar10/data/', train=False, transform=transforms.Compose([transforms.ToTensor(), normalize]), download=False)
...
for epoch in range(learning_epoch):
    ...
    s_grad = []
    for s_i in param['s_params']:
        s = torch.zeros(s_i.shape)
        s_grad.append(s.cuda())
    ...
    for j, (inputs, labels) in enumerate(train_loader, 0):
        ...
        loss.backward()
        for p, s_i in enumerate(param['s_params'], 0):
            s_grad[p].add_(s_i.grad.data)
    for p, s in enumerate(s_grad, 0):
        s_grad[p] = s / inner_epoch
    ...
    new_bn_param = []
    for e_i in param['e_params']:
        new_e_param.append(e_i.detach())
    for s_i in param['s_params']:
        new_s_param.append(s_i.detach())
    for b_i in param['bn_params']:
        new_bn_param.append(b_i.detach())

    param_p = {
        'e_params': new_e_param,
        's_params': new_s_param,
        'bn_params': new_bn_param,
        'L_h0': [],
        'L_c0': [],
        'R_h0': [],
        'R_c0': []
    ...
    for j, (inputs, labels) in enumerate(train_loader, 0):
        if j > inner_epoch:
            break
        inputs, labels = inputs.to(device), labels.to(device)
        outputs = model(inputs, param_p)
        loss1 = loss_function(outputs, labels).cuda()
        global_loss_total += loss1.item()
        global_loss_num += 1

        optimizer.zero_grad(param_p)
        loss1.backward()
        with torch.no_grad():
            pred = outputs.argmax(dim=1)
            total += inputs.size(0)
        ...
        outputs = model(inputs, param)
        loss2 = loss_function(outputs, labels).cuda()
        optimizer.zero_grad(param)
        loss2.backward()

        param_p = optimizer.step(param_p, param, s_grad, hand_e_lr, hand_s_lr)
