...
class RASA(nn.Module):
    ...
    def step(self, param, e_lr, s_lr):
        ...
        for j, s_i in enumerate(param['s_params'], 0):
            ...
            if j==0:
                s_i_grad = s_i.grad
                s_i_grad = s_i_grad.add(s_i.data, alpha=self.decay)
                # e_i_be = e_i
                # e_i.data.add_(e_i_grad, alpha=-e_lr)
                new_s_i = s_i.data - e_lr * s_i_grad
                new_s_i.requires_grad = True
                new_s_i.retain_grad()
                new_s_params.append(new_s_i)
                new_L_params.append(new_s_i)
                new_R_params.append(new_s_i)
                new_L_c.append(new_s_i)
                new_R_c.append(new_s_i)
                continue
