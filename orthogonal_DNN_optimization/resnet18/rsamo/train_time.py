...
def data_preprocess():
    ...
    test_dataset = datasets.CIFAR10(root='./data', train=False, transform=transforms.Compose([transforms.Resize((224, 224)), transforms.ToTensor(), normalize]), download=False)
...
for epoch in range(learning_epoch):
    if epoch % opt.lr_step == 0 and epoch != 0 and meta_e_lr>0.0001:
        meta_e_lr= meta_e_lr * opt.decay_rate
    #if epoch ==3000:
      #  meta_s_lr = 0.05
    if epoch ==6000:
        meta_s_lr = 0.01
