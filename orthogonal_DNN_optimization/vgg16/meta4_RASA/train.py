...
from optimizer.RASA import RASA
from RB import RB
...
def generate_params():
    features = models.vgg16_bn(pretrained=False).features  # .cuda().features
    ...
    for m in features:
        if isinstance(m, nn.Conv2d):
            ...
            # o = torch.randn(3,5)
            # o = nn.init.orthogonal_(o)
            # o1 = torch.matmul(o, o.T)
            # o2 = torch.matmul(o.T, o)
            # otho = m.weight.data
            # otho = otho.view(otho.shape[0], -1)
            # otho1 = torch.matmul(otho, otho.T)
            # thoh2 = torch.matmul(otho.T, otho)
    ...
    for j, s_i in enumerate(s_params):
        ...
        n = M.shape[0]
        r = M.shape[1]
        L_h0_0 = torch.zeros(n, n)
        R_h0_0 = torch.zeros(r, r)
        L_c0_0 = torch.zeros(n, n)
        R_c0_0 = torch.zeros(r, r)
        L_h0.append(L_h0_0.cuda())  # .cuda())
        R_h0.append(R_h0_0.cuda())  # .cuda())
        L_c0.append(L_c0_0.cuda())
        R_c0.append(R_c0_0.cuda())
    params = {
        'e_params': e_params_1,
        's_params': s_params_1,
