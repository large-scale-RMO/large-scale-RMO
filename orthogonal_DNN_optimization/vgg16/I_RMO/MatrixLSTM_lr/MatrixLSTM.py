...
class MatrixLSTM_lr(nn.Module):
    def __init__(self, input_size, output_size):
        ...
        self.lstm1 = MatrixLSTMCell(input_size, output_size).to("cuda:0")

        self.lstm2 = MatrixLSTMCell(input_size, output_size).to("cuda:0")


        # self.proj=nn.Linear((output_size)*(output_size),256)

        self.proj0 = MatrixGrMul(input_size, output_size).to("cuda:0")

        self.proj = nn.Linear(input_size * output_size, 1024).to("cuda:1")
        self.proj2 = nn.Linear(1024, 1).to("cuda:0")
    ...
    def forward(self, input, state):
        ...
        h3 = F.relu(h3)
        # print(h3.shape)
        h3 = h3.view(h3.shape[0], -1).to("cuda:1")
        #print(h3.device)


        output = self.proj(h3)
        output = F.relu(output)
        output = self.proj2(output.to("cuda:0"))
