...
def generate_params():
    ...
    # classifier
    fc_weight_1 = torch.randn(1024, 2048).cuda()
    nn.init.orthogonal_(fc_weight_1)
    e_params_1.append(fc_weight_1)
    fc_bias_1 = torch.randn(1024).cuda()
    fc_bias_1.zero_()
...
for epoch in range(learning_epoch):
    ...
    if epoch % print_epoch_num == 0:
        ...
        print("epoch: ", epoch, "train loss: ", train_loss, "train_accu", 0, 'e_lr: ', meta_e_lr,
              's_lr: ', meta_s_lr)
